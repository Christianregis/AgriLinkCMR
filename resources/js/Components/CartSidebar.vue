      ...
      >
        ...
        <div v-if="cartStore.cartItems.length === 0" class="flex-1 flex items-center justify-center">
          <div class="text-center text-neutral-muted">
            ...
            <button @click="closeCart" class="mt-4 text-brand-primary hover:underline">Continuer vos achats</button>
          </div>
        </div>
        ...
        <div v-else class="flex-1 overflow-y-auto p-6 space-y-6">
          <div v-for="item in cartStore.cartItems" :key="item.product.id" class="flex items-center space-x-4">
            <img
              :src="item.product.primary_image_url || `https://via.placeholder.com/80x80?text=${item.product.title}`"
              alt="Product Image"
            ...
            <div class="flex-1">
              ...
              <p class="text-sm text-neutral-muted">{{ item.product.price }} FCFA / {{ item.product.unit }}</p>
        ...
        <div class="p-6 border-t border-gray-100 space-y-4">
          ...
          <button
            class="w-full py-3 bg-brand-primary text-white font-bold rounded-xl hover:bg-brand-hover transition-colors"
          >
